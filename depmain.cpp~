...
void writeResults() {
	...
	if (!f) {
		f = fopen(out,"w");
		fprintf(f,"exe,instance,n,m,maxnfes,seed,np,finit,fmin,fmax,alpha,heu,ls,frfactor"); //input
	...
	fprintf(f,"%s,%s,%d,%d,%d,%u,%d,%s,%s,%s,%s,%d,%d,%s",
				exe,instance,n,m,maxnfes,seed,np,sfinit,sfmin,sfmax,salpha,heu,ls,sfrfactor);
	//output print
