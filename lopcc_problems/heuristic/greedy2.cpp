...
int main(int argc, char** argv) {
	...
	for (k=n-1; k>0; k--) {
		//1st stage = scan all the unused item to find the best one (jstar)
		...
		//add u[jstar] to x (at position k) and remove from u
		x[k] = u[jstar];
		u[jstar] = u[--nu];
		//2nd stage = try to insert x[k] to all the positions in [k,n) and find the best insertion INS(k,j) with j in [k,n)
		fstar = peval(x,k);
		jstar = k;
		for (j=k+1; j<n; j++) {
			//INS(k,j)
